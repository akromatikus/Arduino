...
void setup() {
  ...
  while (input != 0) {
     ...
     if (Serial.available() > 1) {
       ...
       if (input != 0) {
          guideMotor.step(2048 / (2 * M_PI) * input / armRadius);                   
       }
     }
  }
  ...
  attachInterrupt(digitalPinToInterrupt(2), detectCycle, FALLING); //corresponds to yellow wire (digital) on hall sensor
